...
class anticor2(expert):
    ...
    def get_b(self, data, last_b):
        ...
        for k in np.arange(1,self.window):
            self.exp_w2[k-1,:] = self.update(self.data_day, self.exp_w2[k-1,:].T, k)
    ...
    def update(self, data,last_b, w):
        ...
        if T >= 2*w:
            ...
            mCorr = np.where(sigma!=0, np.divide(mCov,sigma), 0) #set
...
#Create the new portfolio weight adjustments, by adding the j direction weights or the sum by columns and subtracting the i direction weights or the sum by rows
    ...
    def trade(self, data, tc=0, min_period=1):
        ...
        self.exp_ret2 = np.ones((self.window-1,1))
        self.exp_w2 = np.ones((self.window-1, self.window-1))/(self.window-1)
        self.data_day = np.zeros((n,self.window-1))
        ...
        for t in range(n):
            ...
            for k in np.arange(1,self.window):
                self.exp_ret[k-1] = np.dot(self.exp_ret[k-1] * data[k,:],self.exp_w[k-1,:].T)
                self.exp_ret2[k-1] *= self.data_day[t,:].dot(self.exp_w2[k-1,:])
                self.exp_w2[k-1,:] *= self.data_day[t,:] / np.dot(self.data_day[t,:], self.exp_w2[k-1,:].T)
